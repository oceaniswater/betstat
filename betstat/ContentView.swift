                ...
                HStack {
                    VStack(alignment: .leading) {
                        Rectangle()
                            ...
                            .foregroundColor(.green)
                            .opacity(/*@START_MENU_TOKEN@*/0.8/*@END_MENU_TOKEN@*/)
                        Text(vm.getPercentWins(company) > 15 ? "\(company.wins) (\(vm.getPercentWins(company))%)" : "\(company.wins)")
                            .font(Font.footnote.weight(.bold))
                    ...
                    VStack(alignment: .leading) {
                        Rectangle()
                            ...
                            .foregroundColor(.red)
                            .opacity(0.8)
                        Text(vm.getPercentLoses(company) > 15 ? "\(company.loses) (\(vm.getPercentLoses(company))%)" : "\(company.loses)")
                            .font(Font.footnote.weight(.bold))
                            .minimumScaleFactor(0.8)
                            .fixedSize(horizontal: true, vertical: false)
                    }
                    VStack(alignment: .trailing) {
                        Rectangle()
                            ...
                            .foregroundColor(.gray)
                            .opacity(/*@START_MENU_TOKEN@*/0.8/*@END_MENU_TOKEN@*/)
                        Text(vm.getPercentReturns(company) > 15 ? "\(company.returns) (\(vm.getPercentReturns(company))%)" : "\(company.returns)")
                            .font(Font.footnote.weight(.bold))
                            .minimumScaleFactor(0.8)
                            .fixedSize(horizontal: true, vertical: false)
